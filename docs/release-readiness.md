...
| Release gates | ready | Default CI gate, full compatibility matrix, Cargo/npm package checks, reporter/config/CLI/blame gates. The default gate prints per-step timings, caches Cargo/pnpm/upstream build artifacts, and uses local prebuilt npm package smoke without install-time Cargo builds. GitHub npm/crates publish workflows now run `scripts/release-candidate.sh` before publishing. |
